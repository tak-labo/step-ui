...
**`lib/auth.ts`** — NextAuth v5 Credentials Provider。`UI_PASSWORD_HASH` 環境変数は bcrypt hash を Base64 エンコードした値。Docker Compose が env 値中の `$` を変数展開するため、そのまま設定すると壊れる。
...
| `caddy-data` | Caddy の ACME 証明書キャッシュ（Let's Encrypt/Cloudflare DNS チャレンジ） |
| `caddy-config` | Caddy の設定・実行時データ |
...
本番公開では `docker compose up --build` で Caddy を入口にし、`.env` の `CADDY_ENABLED=true`、`CLOUDFLARE_API_TOKEN=<token>`、実ホスト名の `PUBLIC_DOMAIN` で有効化する。`PUBLIC_URL` が未設定なら `https://${PUBLIC_DOMAIN}` を step-ui の外向け URL に使う。
Caddy は Cloudflare の DNS API チャレンジで Let's Encrypt 証明書を自動取得する（`docker/Caddyfile` で設定）。
